...
from tensorflow.keras.layers import Conv1D, MaxPooling1D, Embedding#, Merge, 
...
# ##################################################################################
# bi LSTM with multihead attention ##############################

    model.add(Bidirectional(LSTM(embed_size)))
              
    model.add(customAttLayer())
    model.add(customAttLayer())
    model.add(customAttLayer())
